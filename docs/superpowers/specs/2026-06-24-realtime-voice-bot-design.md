...
talk-ai/
  ...
  handlers/
    research.py
    coding.py              # Claude Code CLI 駆動 + 自動リカバリ・ウォッチドッグ
    appop/
      __init__.py
      mac.py               # osascript / shortcuts
      apple_notes.py       # Apple メモ操作 + 来歴台帳(LLM作成分のみ変更可)
      mcp.py               # Calendar / Drive
      discord_ops.py       # チャンネル移動・BGM・ロール
      obsidian.py          # vault 直接編集
      vault_selector.py    # 3系統(口頭/キーワード/小型LLM)選択 + 照合
      vault_classifier/
        model.py           # 専用小型LLM(bonsai系)推論・バージョン管理
        trainer.py         # 不一致サンプル蓄積・ファインチューニング実行
```
...
- Obsidian vault 登録表の初期内容(各 vault のパスと用途・命名規約)
- vault 分類用「bonsai」小型モデルの正体・特定(極小ローカル LLM 想定だが要確認)
- ファインチューニングの実行契機(不一致ごと即時 / バッチ蓄積後に定期)とデータ形式
