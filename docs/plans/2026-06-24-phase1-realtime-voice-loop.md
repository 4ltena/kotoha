...
> 設計書 §8 のモジュールは Python パッケージとして `talk_ai/` 配下に置く(ハイフン不可のため)。例: 設計書の `voice/receiver.py` → `talk_ai/voice/receiver.py`。フェーズ1スコープ外の `turntaking/`・`dispatch/`・`handlers/`・`tone_adjuster.py` は本計画では作らない。
...
- Create: `talk_ai/__init__.py`
- Create: `talk_ai/voice/__init__.py`
- Create: `talk_ai/llm/__init__.py`
- Create: `talk_ai/config.py`
...
Expected: FAIL (`ModuleNotFoundError: No module named 'talk_ai'`)
> Step 0 で pytest を入れているので、赤の理由は `pytest` 未インストールではなく `talk_ai` 未作成になる(点17)。
...
`talk_ai/__init__.py`, `talk_ai/voice/__init__.py`, `talk_ai/llm/__init__.py`: 空ファイル。
...
```bash
git add pyproject.toml requirements.txt talk_ai/ tests/test_config.py
...
**Files:**
- Create: `talk_ai/voice/audio_utils.py`
...
Expected: FAIL (`ModuleNotFoundError: No module named 'talk_ai.voice.audio_utils'`)

- [ ] **Step 3: 最小実装**

`talk_ai/voice/audio_utils.py`:
```python
import numpy as np
...
```bash
git add talk_ai/voice/audio_utils.py tests/voice/test_audio_utils.py
...
Expected: FAIL (`ModuleNotFoundError: No module named 'talk_ai.voice.vad'`)

- [ ] **Step 3: 最小実装**

`talk_ai/voice/vad.py`:
```python
...
```bash
git add talk_ai/voice/vad.py tests/voice/test_vad.py
...
**Files:**
- Modify: `talk_ai/voice/vad.py`(`BargeInDetector` を追記)
...
```bash
git add talk_ai/voice/vad.py tests/voice/test_bargein_detector.py
...
Expected: FAIL (`ModuleNotFoundError: No module named 'talk_ai.voice.stt'`)

- [ ] **Step 3: 最小実装**

`talk_ai/voice/stt.py`:
```python
import numpy as np
...
```bash
git add talk_ai/voice/stt.py tests/voice/test_stt.py
...
**Files:**
- Create: `talk_ai/voice/receiver.py`
...
import types
import numpy as np
from talk_ai.voice.receiver import PerUserSink
...
Expected: FAIL (`ModuleNotFoundError: No module named 'talk_ai.voice.receiver'`)

- [ ] **Step 3: 最小実装**

`talk_ai/voice/receiver.py`:
```python
...
```bash
git add talk_ai/voice/receiver.py tests/voice/test_receiver.py
...
**Files:**
- Create: `talk_ai/llm/persona.py`
- Create: `talk_ai/llm/front_client.py`
...
Expected: FAIL (`ModuleNotFoundError: No module named 'talk_ai.llm.persona'`)

- [ ] **Step 3: 最小実装(persona)**

`talk_ai/llm/persona.py`:
```python
SYSTEM_PROMPT = (
    "あなたは音声で雑談する気さくな相棒です。"
...
```bash
git add talk_ai/llm/persona.py tests/llm/test_persona.py
...
Expected: FAIL (`ModuleNotFoundError: No module named 'talk_ai.llm.front_client'`)

- [ ] **Step 7: 最小実装(front_client)**

`talk_ai/llm/front_client.py`:
```python
import json
...
```bash
git add talk_ai/llm/front_client.py tests/llm/test_front_client.py
...
**Files:**
- Create: `talk_ai/llm/sentence_splitter.py`
...
Expected: FAIL (`ModuleNotFoundError: No module named 'talk_ai.llm.sentence_splitter'`)

- [ ] **Step 3: 最小実装**

`talk_ai/llm/sentence_splitter.py`:
```python
...
```bash
git add talk_ai/llm/sentence_splitter.py tests/llm/test_sentence_splitter.py
...
```python
import pytest
from talk_ai.voice.tts import synthesize
...
async def test_synthesize_default_real_tts_http():
    from talk_ai.voice.tts import synthesize_default
...
Expected: FAIL (`ModuleNotFoundError: No module named 'talk_ai.voice.tts'`)

- [ ] **Step 3: 最小実装**

`talk_ai/voice/tts.py`:
```python
import aiohttp
...
```bash
git add talk_ai/voice/tts.py tests/voice/test_tts.py
...
**Files:**
- Create: `talk_ai/voice/playback.py`
...
```python
import pytest
from talk_ai.voice.playback import Player
...
Expected: FAIL (`ModuleNotFoundError: No module named 'talk_ai.voice.playback'`)

- [ ] **Step 3: 最小実装**

`talk_ai/voice/playback.py`:
```python
import asyncio
import io
from typing import Callable, Optional
...
```bash
git add talk_ai/voice/playback.py tests/voice/test_playback.py
...
**Files:**
- Create: `talk_ai/orchestrator.py`
...
- Produces: `class Orchestrator: __init__(self, *, transcriber, llm_stream, tts, player, model: str, vad_factory, persona=<talk_ai.llm.persona>, history_max_turns=20, vad_threshold=0.5, vad_silence_ms=400, bargein_trigger_ms=250, sample_rate=SAMPLE_RATE_HZ, vad_window=VAD_WINDOW_SAMPLES, fallback_text="...", stt_timeout=30.0, tts_timeout=15.0, play_timeout=60.0, splitter_factory=SentenceSplitter, loop=None)`。本タスクで実装するメソッド: `async handle_utterance(self, user_id, audio) -> None`, `async _run_turn(self, messages) -> None`(及び内部の 3 段パイプラインコルーチン `_llm_to_sentences` / `_sentences_to_audio` / `_audio_to_playback` と `_speak_fallback`)。属性 `self.history: collections.deque`。
...
```python
import numpy as np
from talk_ai.orchestrator import Orchestrator
from talk_ai.llm import persona
...
Expected: FAIL (`ModuleNotFoundError: No module named 'talk_ai.orchestrator'`)

- [ ] **Step 3: 最小実装**

`talk_ai/orchestrator.py`:
```python
import asyncio
import logging
...
from talk_ai.llm import persona as _persona
from talk_ai.llm.sentence_splitter import SentenceSplitter
# Task 12 で feed_audio から使用
from talk_ai.voice.vad import VadSegmenter, BargeInDetector
...
```bash
git add talk_ai/orchestrator.py tests/test_orchestrator.py
...
**Files:**
- Modify: `talk_ai/orchestrator.py`(`request_bargein` / `feed_audio` / `_route_audio` / VAD 生成ヘルパを追記)
- Create: `talk_ai/health.py`(起動時の疎通チェック)
- Create: `talk_ai/bot.py`(Discord 接続のエントリポイント。手動/結合確認用)
...
```python
from talk_ai.health import check_services
...
```python
from talk_ai.orchestrator import make_on_audio
...
`talk_ai/orchestrator.py` の `Orchestrator` にメソッドを追記:
```python
...
`talk_ai/bot.py`(手動/結合確認用エントリ。単体テスト対象外):
```python
import asyncio
import functools
import logging
...
from talk_ai.voice.playback import Player
from talk_ai.voice.receiver import PerUserSink
from talk_ai.voice.stt import Transcriber, build_whisper
from talk_ai.voice.tts import synthesize
from talk_ai.voice.vad import SileroVad
...
```bash
git add talk_ai/orchestrator.py talk_ai/health.py talk_ai/bot.py tests/test_bargein.py tests/test_health.py tests/test_wiring.py
...
- [ ] 実 VC 手動確認(設計書 §10): `python -c "from talk_ai.bot import run_bot; from talk_ai.config import Config; run_bot('<TOKEN>', <CHANNEL_ID>, Config())"` で VC に参加し、
...
- 依存の段階導入(点17/18): base=aiohttp+numpy、extra=voice/ml/dev/all。Task 1 は pytest だけで赤→緑、重い ML は Task 3 以降で導入。bootstrap ステップ追加で最初の赤理由を `talk_ai` 未作成に固定。
