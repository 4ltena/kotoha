...
| 用途 | 採用 | ライセンス | 備考 |
| --- | --- | --- | --- |
| 知覚 VLM（通常時・既定） | Qwen3-VL-4B-Instruct | Apache-2.0 | 約 3.3GB(q4)。Ollama 公式タグ `qwen3-vl:4b`。VII に常駐 |
...
- **Radeon VII（Vulkan・非リアルタイム）**。遅延を許せる思考と視覚。記憶の圧縮・要約、関係性分析、通常モードの知覚 VLM。

Radeon VII は gfx906 で ROCm の公式サポートから外れている。今回は ROCm を使わず **Vulkan** で動かす。VII 側は llama.cpp の Vulkan ビルドを OpenAI 互換サーバ（llama-server）として常駐させ、Qwen3-VL の画像入力は mmproj を併用する。デバイス指定は `GGML_VK_VISIBLE_DEVICES`、待受ポートは 4080 側の Ollama と別にする。kotoha 本体は base_url を VII のサーバへ向けるだけで、サーバ実装の差は意識しない。
...
- **ゲームモード**。ゲーム起動を検知したら自動で切り替える。挙動は設定で 2 択。
  ...
  - **リアルタイム型**。知覚を高頻度化し（既定 0.5 秒、設定可変）、画面状況を逐次把握する。Windows では dxcam でゲーム画面を取得する。VLM 推論は VII で回し、4080 をゲームに残す。VII が追いつかない場合に 4080 へ寄せる設定も持つ。
...
- VII の Vulkan サーバ構成（llama-server の mmproj 指定、NVIDIA ＋ AMD 同居時の Vulkan 列挙順、Qwen3-VL-4B の gfx906 上の実効スループット）は実証フェーズで確定する。
