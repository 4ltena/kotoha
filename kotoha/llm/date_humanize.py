...
def greeting_time_guidance(text: str, now) -> str:
    ...
    if "こんにちは" in text or "こんにちわ" in text:
        # 昼の挨拶。夕方は許容し、夜・深夜だけ戸惑う(誤検出を避け保守的に)。
        if band in ("夜", "深夜"):
            return (
                "ユーザーは昼の挨拶「こんにちは」と言ったが、現在の時間帯は"
                f"「{band}」。返答では現在の時間帯を使い、短く理由を尋ねる。"
            )
    if "こんばんは" in text or "こんばんわ" in text:
        # 夜の挨拶。夕方は許容し、朝・昼だけ戸惑う。
        if band in ("朝", "昼"):
            return (
                "ユーザーは夜の挨拶「こんばんは」と言ったが、現在の時間帯は"
                f"「{band}」。返答では現在の時間帯を使い、短く理由を尋ねる。"
            )
    return ""
